            ...
            if (intervalTime > 0)
            {
                ...
                if (outFile && outFile->is_open())
                {
                    *outFile << currentTime << "\t" << throughputMbps << std::endl;
                    ...
                    NS_LOG_UNCOND("UE " << g_ueIdToSliceName[ueNode->GetId()] << " (Node ID: " << ueNode->GetId()
                                        << ") Throughput: " << throughputMbps << " Mbps at time " << currentTime
                                        << "s");
                }
            }
...
main(int argc, char* argv[])
{
    ...
    // --- Traffic Configuration for Slices to Highlight Delay Differences ---
    // URLLC: Small packets, high frequency (low interval) for low latency
    Time urllcInterval = MicroSeconds(50); // Increased from 20 to 50 for more distinct difference
    uint32_t urllcPacketSize = 45;

    // eMBB: Large packets, moderate frequency for high throughput
    Time embbInterval = MilliSeconds(1); // Reduced interval from 20ms for higher load and potential queueing
    uint32_t embbPacketSize = 1400; // Standard MTU size often around 1400-1500 bytes

    // mMTC: Small packets, low frequency (high interval) for sparse communication
    Time mmtcInterval = MilliSeconds(50); // Increased interval from 20ms to make it sparser
    uint32_t mmtcPacketSize = 100; // Small packet size
    ...
    for (uint32_t u_idx = 0; u_idx < numUePerSlice; ++u_idx)
    {
        ...
        dlClient.SetAttribute("Interval", TimeValue(urllcInterval));
        dlClient.SetAttribute("MaxPackets", UintegerValue(UINT32_MAX));
        dlClient.SetAttribute("PacketSize", UintegerValue(urllcPacketSize));
        clientApp.Add(dlClient.Install(remoteHost));
        NS_LOG_UNCOND("UE " << g_ueIdToSliceName[ueNode->GetId()] << " (Node ID: " << ueNode->GetId()
                            << ") assigned to URLLC slice. Interval: " << urllcInterval
                            << ", PacketSize: " << urllcPacketSize);
    }
    ...
    for (uint32_t u_idx = numUePerSlice; u_idx < 2 * numUePerSlice; ++u_idx)
    {
        ...
        UdpClientHelper dlClient(ueIpIface.GetAddress(u_idx), portUdp);
        dlClient.SetAttribute("Interval", TimeValue(embbInterval));
        dlClient.SetAttribute("MaxPackets", UintegerValue(UINT32_MAX));
        dlClient.SetAttribute("PacketSize", UintegerValue(embbPacketSize));
        clientApp.Add(dlClient.Install(remoteHost));
        NS_LOG_UNCOND("UE " << g_ueIdToSliceName[ueNode->GetId()] << " (Node ID: " << ueNode->GetId()
                            << ") assigned to eMBB slice. Interval: " << embbInterval
                            << ", PacketSize: " << embbPacketSize);
    }
    ...
    for (uint32_t u_idx = 2 * numUePerSlice; u_idx < nUeNodes; ++u_idx)
    {
        ...
        UdpClientHelper dlClient(ueIpIface.GetAddress(u_idx), portUdp);
        dlClient.SetAttribute("Interval", TimeValue(mmtcInterval));
        dlClient.SetAttribute("MaxPackets", UintegerValue(UINT32_MAX));
        dlClient.SetAttribute("PacketSize", UintegerValue(mmtcPacketSize));
        clientApp.Add(dlClient.Install(remoteHost));
        NS_LOG_UNCOND("UE " << g_ueIdToSliceName[ueNode->GetId()] << " (Node ID: " << ueNode->GetId()
                            << ") assigned to mMTC slice. Interval: " << mmtcInterval
                            << ", PacketSize: " << mmtcPacketSize);
    }
    ...
    Simulator::Schedule(Seconds(g_reportingInterval), &CalculateThroughput, std::cref(ueNodes),
                        Seconds(g_reportingInterval));
    ...
    if (run)
    {
        ...
        Simulator::Stop(Seconds(simTime)); // Stop the simulation based on simTime
        Simulator::Run();
    }
